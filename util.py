...
# stop_words = preprocess_stopWords()
...
def load_model(sess, args):
    ...
    saver.restore(sess, args.model_save_path)
    # saver.restore(sess, './model/model.checkpoint')


def load_model2(sess, checkpoint_path):
    """
...
def findMaxminposition(element):
    return np.argmax(element)
...
def save_predictions(pred, file):
    ...
    with open(file, 'w') as csvfile:
        # fieldnames = ['Stance', 'Probability','news_ID']
        fieldnames = ['news_ID']
        writer = DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()
        ...
        for instance in pred:
            # print(instance)
            news_num = "{:06d}".format(num + 1)
            num += 1
            news_id = 'news_' + str(news_num)
            max_position = findMaxminposition(instance)
            max_position_probability = instance[max_position]
            pair = (label_ref_rev[max_position],max_position_probability,news_id)
            sort_list.append(pair)

        
        sorted_pair = sorted(sort_list,key= itemgetter(0,1),reverse = True)
        


        for tuple in sorted_pair[:300]:
            tuple_dict = {}
            # tuple_dict['Stance'] = tuple[0]
            # tuple_dict['Probability'] = tuple[1]
            tuple_dict['news_ID'] = tuple[2]
            writer.writerow(tuple_dict)
